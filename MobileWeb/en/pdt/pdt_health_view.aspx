<%@ Page Language="C#" MasterPageFile="/en/EnSiteSub.Master" AutoEventWireup="true" CodeBehind="pdt_health_view.aspx.cs" Inherits="MobileWeb.en.pdt.pdt_health_view" %>

<asp:Content ID="Content" ContentPlaceHolderID="ContentPlaceHolder" runat="server">

	<header class="header">
		<script src="<%=GetRoot() %>/en/common/include/header.js" ></script> 	
		<h1 class="acc-hidden">Health Suppliments</h1>	
		<p class="location">
			<span>Products</span><em>></em>
			<strong>Health Suppliments</strong>
		</p>
	</header>
			...
			<div id="slider3_btns" class="view_slide_box w600">
				...
				<ul id="slider1" >
                    <% 
                        ...
                        { 
                    %>
					<li><img src="<%=GetData(0, "PROD_IMG1")%>" alt="" onerror="this.src='<%=GetRoot() %>/en/common/images/products/no_big.jpg'"/></li>
                    <%
                        } 
                        if (GetData(0, "PROD_IMG2").Equals("") == false)                            
                        { 
                    %>
					<li><img src="<%=GetData(0, "PROD_IMG2")%>" alt="" onerror="this.src='<%=GetRoot() %>/en/common/images/products/no_big.jpg'"/></li>
                    <%
                        } 
                        if (GetData(0, "PROD_IMG3").Equals("") == false)                            
                        { 
                    %>
					<li><img src="<%=GetData(0, "PROD_IMG3")%>" alt="" onerror="this.src='<%=GetRoot() %>/en/common/images/products/no_big.jpg'"/></li>
                    <%
                        } 
                        if (GetData(0, "PROD_IMG4").Equals("") == false)                            
                        { 
                    %>
					<li><img src="<%=GetData(0, "PROD_IMG4")%>" alt="" onerror="this.src='<%=GetRoot() %>/en/common/images/products/no_big.jpg'"/></li>
                    <%
                        }
                        if (GetData(0, "PROD_IMG5").Equals("") == false)
                        { 
                    %>
					<li><img src="<%=GetData(0, "PROD_IMG5")%>" alt="" onerror="this.src='<%=GetRoot() %>/en/common/images/products/no_big.jpg'"/></li>
                    <% } %>
				</ul>
			</div>	
			<div class="sns w600"> 
