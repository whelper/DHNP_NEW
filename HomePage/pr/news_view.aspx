<%@ Page Language="C#" MasterPageFile="~/SiteSub.Master" AutoEventWireup="true" CodeBehind="news_view.aspx.cs" Inherits="HomePage.pr.news_view" %>
<asp:Content ID="Content" ContentPlaceHolderID="ContentPlaceSubLayer" runat="server">

<section class="sub_content" >
	<h1 class="pageTitle"> 새소식    </h1>
	<p class="location">
		...
		<a href="/pr/news_list.aspx"><strong>새소식</strong></a>
	</p>
	<!--  내용  ***************** -->
				...
				<td  colspan="8">
					<div  class="view_adit">
						<p><%=GetData(0, 0, "CONT")%></p>
                         ...
                         <% if ("".Equals(GetData(0, 0, "CLIP_URL")) == false)
                       {%> 
                         <p class="video-container">
                            <iframe width="560" height="315" src="https://www.youtube.com/embed/<%=GetData(0, 0, "CLIP_URL") %>" frameborder="0" allowfullscreen></iframe>
                        </p>
                        <%} %>
					</div>	 
					<!-- // view_adit -->
	...
	<p class="listview_btns">
        <%
            ...
            {
        %>
		<button type="button" class="btn_view_prev" onclick="location.href='news_view.aspx?seq_no=<%=GetData(1, 0, "PREV_SEQ_NO")%>'">이전글</button>
        <% 
            }
        %>
        <%
            ...
            {
        %>
		<button type="button" class="btn_view_next" onclick="location.href='news_view.aspx?seq_no=<%=GetData(2, 0, "NEXT_SEQ_NO")%>'">다음글</button>
        <% 
            }
        %>
		<button type="button" onclick="javascript:location.href='news_list.aspx'" class="btn_view_list">목록보기</button>
	</p>
	<!-- // 내용 ***************** -->         
</section>  <!--// sub_content -->
