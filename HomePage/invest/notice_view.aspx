<%@ Page Language="C#" MasterPageFile="~/SiteSub.Master" AutoEventWireup="true" CodeBehind="notice_view.aspx.cs" Inherits="HomePage.invest.notice_view" %>
<asp:Content ID="Content" ContentPlaceHolderID="ContentPlaceSubLayer" runat="server">

<section id="content" class="sub_content" >
	<header>
		<h1 class="pageTitle"> 공고사항  </h1>
		<p class="location">
	...
	<table class="table_view" border="1" >
		<caption class="hidden">  공지/공고 상세보기   </caption>
		<colgroup>
			<col style="width:12%" >
			<col style="width:13%" >
			<col style="width:12%" >
	...
	<p class="listview_btns">
		<%
            ...
            {
        %>
		<button type="button" class="btn_view_prev" onclick="location.href='notice_view.aspx?seq_no=<%=GetData(1, 0, "PREV_SEQ_NO")%>'">이전글</button>
        <% 
            }
        %>
        <%
            ...
            {
        %>
		<button type="button" class="btn_view_next" onclick="location.href='notice_view.aspx?seq_no=<%=GetData(2, 0, "NEXT_SEQ_NO")%>'">다음글</button>
        <% 
            }
        %>
